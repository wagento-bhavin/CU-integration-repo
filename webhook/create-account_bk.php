...
curl_setopt($ch, CURLOPT_URL, 'https://api-staging.cheddarup.com/api/clients/users');
...
fwrite($fp,$token);
fclose($fp);
$sql = "INSERT INTO cheddarup_users (email, cheddarup_id, uuid, created_at, updated_at, token)
VALUES ('{$email}','{$id}','{$uuid}','{$created}','{$updated}','{$token}')";
...
$collection_name = $first_name . "’s PixieLane Online Store";
	...
	curl_setopt($ch, CURLOPT_URL, 'https://api-staging.cheddarup.com/api/users/tabs');
	curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
	curl_setopt($ch, CURLOPT_HTTP_VERSION, 'CURL_HTTP_VERSION_1_1');
