...
$categories['Men']    = ['Tops'];
$shopify_tags = array();
$shopify_tags['Girls'] = ['Tops', 'dresses', 'Bottoms', 'Jumpsuits', 'swimwear', 'Loungewear', 'Sets'];
$shopify_tags['Boys'] = ['Tops', 'Bottoms', 'Sets'];
$shopify_tags['Babies'] = ['Tops', 'dresses', 'Bottoms', 'Romper', 'Sets'];
$shopify_tags['Women'] = ['Tops', 'Bottoms', 'Jumpsuits', 'Sweatshirts & Cardigans', 'Sets'];
...
$count_url = "https://4d87206512af68fc951ce70348bb8105:10ee2c6ba02eef12fdda08912d318c49@pixielanekidz.myshopify.com/admin/api/2019-04/products/count.json?published_status=published";
$total = file_get_contents($count_url);
$total = json_decode($total, true);
$total = $total['count'];
$page = ceil($total / 250);
$count = 0;
echo "Total " . $total . " and Pages are " . $page . "\n";
for ($i = 1; $i <= $page; $i++) {
    $url = "https://4d87206512af68fc951ce70348bb8105:10ee2c6ba02eef12fdda08912d318c49@pixielanekidz.myshopify.com/admin/api/2019-04/products.json?limit=250&published_status=published&page=" . $i;
                ...
                if (count(preg_grep('/\b' . $cat . '\b/i', $tags)) > 0 && count(array_intersect($tags, $sizes[$cat])) > 0) {
                    ...
                    foreach ($shopify_tags[$product_cat] as $key => $subcat) {
                        if (count(preg_grep('/\b' . $subcat . '\b/i', $tags)) > 0) {
                            ...
                            $result = json_decode(curl_exec($ch), true);
                            $httpcode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                            print_r("-2--" . $httpcode. 'for '.end($image_ext)."\n");
                            curl_close($ch);
                            if ($httpcode == 200) {
                            ...
                            // print_r($b64image);
                            $image_header = array(
                                'Content-Type: image/' . end($image_ext),
