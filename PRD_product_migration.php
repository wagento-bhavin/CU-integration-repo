				...
				if(count(preg_grep('/\b'.$cat.'\b/i', $tags)) > 0 && count(array_intersect($tags, $sizes[$cat])) > 0){
					...
					foreach ($shopify_tags[$product_cat] as $key=>$subcat) {
						if(count(preg_grep('/\b'.$subcat.'\b/i', $tags)) > 0){
							...
							$url = 'https://api.cheddarup.com/api/users/tabs/'.$col_ids[$product_cat].'/items';
					    	...
					    	$signedurl = "";
					    	$uploadPath = "";
							$url = 'https://api.cheddarup.com/api/users/tabs/'.$col_ids[$product_cat].'/items/'.$item_id.'/signed_upload_url';
							...
							print_r("-3--".$httpcode);
							if($httpcode == 403){
								$file = 'err_products.txt';
								$err_product = $product['title']."\n";
								file_put_contents($file, $err_product, FILE_APPEND | LOCK_EX);
							}
							// print_r("-3*--".$variant_image_url);
							// print_r("-3**--".$signedurl);
							//Create Database Record for image
							$url = 'https://api.cheddarup.com/api/users/tabs/'.$col_ids[$product_cat].'/items/'.$item_id.'/create_image_record';
							...
							$url = 'https://api.cheddarup.com/api/users/tabs/'.$col_ids[$product_cat].'/items/'.$item_id;
					        $ch = curl_init();
					        curl_setopt($ch, CURLOPT_URL, $url);
					        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
					        curl_setopt($ch, CURLOPT_HTTP_VERSION, 'CURL_HTTP_VERSION_1_1');
